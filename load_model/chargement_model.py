...
class Painter:
    ...
    def plotBoxAndItems(self,title="",alpha=0.2,write_num=False,fontsize=10, color_types={}):
        ...
        for item in self.items:
            ...
            if color in color_types:
              legend_colors[color] = color_types[color]
        ...
        if legend_colors:
          patches = [mpatches.Patch(color=color, label=box_type) for color, box_type in legend_colors.items()]
          plt.legend(handles=patches, loc='upper right', title="Box Types")
...
def Model(n_bins, bins, n_boxes, boxes, draw_result=False, print_result=False, color_types={}):
  start = time.time()

  packer = Packer()

  for i in range(n_bins):
    packer.addBin(bins[i])

  for j in range(n_boxes):
    packer.addItem(boxes[j])
  ...
  for idx, b in enumerate(packer.bins):
    ...
    for item in b.items:
      res_item = {}
      res_item['partno'] = item.partno
      res_item['color'] = item.color
      res_item['position'] = item.position
      res_item['rotation_type'] = item.rotation_type
      res_item['W*H*D'] = str(item.width) +' * '+ str(item.height) +' * '+ str(item.depth)
      res_item['volume'] = float(item.width) * float(item.height) * float(item.depth)
      res_item['weight'] = float(item.weight)
      res_bin['items'].append(res_item)
    ...
    res_bin['space_usage'] = round(volume_t / float(volume) * 100 ,2)
    res_bin['residual_volumn'] = float(volume) - volume_t
    res_bin['gravity_distribution'] = b.gravity

    fitted_result.append(res_bin)
  ...
  for item in packer.unfit_items:
    ...
    unfitted_result1.append(res_item)

  unfitted_result = [unfitted_result1, {'unpacked item volume': volume_f}]
  ...
  result = [fitted_result, unfitted_result, used_time]
  return result

list_bins = [
    Bin('example7-Bin1', (5, 5, 5), 100,0,0),
...
colors= {'brown': 'produit1', 'pink': 'produit2', 'olive': 'produit3', 'yellow': 'produit4'}

fitted_result, unfitted_result, used_time = Model(len(list_bins), list_bins, len(list_boxes), list_boxes, draw_result=True, print_result=False, color_types=colors)
